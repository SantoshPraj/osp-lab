...
for i in anshosts:
    ...
    time.sleep(10)
    chansssh.expect(':~#')
    vdelete = ("virsh undefine ans-{} --remove-all-storage".format(ipstart))
    chansssh.sendline(vdelete)
    chansssh.expect(':~#')
    print ("\n Output of vdelete command: {}".format(chansssh.before.decode()))
    if chansssh.isalive():
...
for i in conthosts:
    ...
    time.sleep(10)
    chcontssh.expect(':~#')
    vdelete = ("virsh undefine cont-{} --remove-all-storage".format(ipstart))
    chcontssh.sendline(vdelete)
    chcontssh.expect(':~#')
    print ("\n Output of vdelete command: {}".format(chcontssh.before.decode()))
...
for i in strhosts:
    ...
    chstrssh.expect(':~#')
    print ("\n Output of virsh undefine command: {}".format(chstrssh.before.decode()))
    if chstrssh.isalive():
        print (colors.fg.blue,"\n The child process is still alive, closing connection to {}".format(i),colors.reset)
    #Incrementing the IP block within the for loop
    ipstart+=1
